...
# INSTRUMENT_STOCK_PHOTOS / INSTRUMENT_KEYWORDS / _detect_instruments moved
# to mock_templates_music.py (only music/collective's card row used them).
...
# Category-specific signature-section renderers (day-timeline, admissions-path,
# preschool-detail, explorer-spotlight, community-reasons, lesson-scroll,
# showcase-marquee, collective-lineup, academy-path, stat-block, parent-qa,
# camp-calendar, team-roster) moved to mock_templates_preschool.py,
# mock_templates_music.py, and mock_templates_sports.py — see _render_variant_body
# below, which dispatches to each module's build_body().

# Imported here (not at module top) because these submodules import this
# module back (`from scripts import generate_website_mocks as core`) to
# reach shared rendering helpers — importing them only once everything they
# need is already defined above avoids a circular-import error at load time.
from scripts import mock_templates_music, mock_templates_preschool, mock_templates_sports  # noqa: E402


# Each category's signature-section content, and the specific (hero shape,
# signature, detail, enrollment shape) combination chosen per variant, lives
# in its own module — see mock_templates_preschool.py's docstring for why.
# No two siblings in the same category repeat the same (hero, close) pair or
# the same band, and the pairing is chosen for fit — e.g. a curriculum-path
# variant closes with numbered steps, not a form — not shuffled for coverage
# alone. The typeface pairing, header shape, page shell and footer come from
# VARIANT_STYLE and are likewise unique within a category.
CATEGORY_BODY_BUILDERS = {
    "preschool": mock_templates_preschool.build_body,
    "music": mock_templates_music.build_body,
    "sports": mock_templates_sports.build_body,
}


def _render_variant_body(ctx: dict, items: list[tuple[str, str]]) -> str:
    type_id = ctx["type_id"]
    builder = CATEGORY_BODY_BUILDERS.get(type_id, mock_templates_sports.build_body)
    ctx, hero, signature, detail, enrollment, layout_class = builder(ctx, items)
